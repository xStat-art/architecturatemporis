 ...
 * tia-bar-legacy.js
 * ─────────────────────────────────────────────────────────────────────────
 * Split off from tia-bar.js on 20 Aug 2026. tia-bar.js was rewritten on
 * 19 Aug 2026 (commit "Sync simulator, i18n, tia-bar/ring, zone-profile,
 * intervention-entry (24 Aug Kotor demo prep)") to a different concept —
 * a before/after TIA-scenario bar driven by the server's computeTIA
 * response — and no longer exports computeZoneStats. tia.html's own inline
 * script still imported computeZoneStats from tia-bar.js, so that import
 * started throwing on load ("does not provide an export named
 * 'computeZoneStats'") and the whole module script aborted before it ever
 * reached renderZone('hn001') — this is why the chart area went completely
 * blank with no partial render and no on-page error.
 *
 * tia.html is a standalone Zone 001 pilot-dataset demo (see its own TODO:
 * "replace with a live fetch ... once this view moves off the pilot
 * dataset") — it was never meant to track tia-bar.js's move to the real
 * before/after scenario API, so rather than reverting tia-bar.js (which
 * would undo Sergei's 19 Aug work prepping the 24 Aug Kotor demo), this
 * file is an exact copy of tia-bar.js as it stood before that rewrite,
 * kept only for tia.html to import. Do not add new consumers of this file
 * — new work should use tia-bar.js's before/after API instead.
 *
...
export function renderTIABar(containerId, data) {
  ...
  if (!container) { console.error(`tia-bar-legacy: no element #${containerId}`); return; }
